/**
 * @file mainSixLedsButtons.c
 * @author Pedro Botelho (pedrobotelho15@alu.ufc.br)
 * @brief Toggle the 4 embedded LEDs and 2 external LEDs with a button.
 * @version 1.0
 * @date 2022-07-02
 * 
...
void ledInit( ){
	...
	HWREG(SOC_CM_PER_REGS + CM_PER_GPIO2) |= CM_PER_GPIO2_CLKCTRL_OPTFCLKEN_GPIO_2_GDBCLK | CM_PER_GPIO2_CLKCTRL_MODULEMODE_ENABLE;
   	...
   	HWREG(SOC_CONTROL_REGS + CM_conf_gpmc_ad12) |= (1U << 5) | 7;
...
void ledToggle( ){
		flagBlink = ((HWREG(SOC_GPIO_1_REGS + GPIO_DATAOUT) & (LED_EXT_01 | LED_01 | LED_02 | LED_03 | LED_04) 
			&& (HWREG(SOC_GPIO_2_REGS + GPIO_DATAOUT) & (LED_EXT_02))));
		if(!flagBlink) {
